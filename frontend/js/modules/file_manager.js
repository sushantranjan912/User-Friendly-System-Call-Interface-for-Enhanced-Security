...
// Client-side validation rules (keep in sync with backend `Config`)
...
function validateFilenameClient(filename) {
  ...
  if (
    ...
    return false;
  // Normalize simple disallowed chars — only allow typical safe names
  const safe = filename.replace(/[^a-zA-Z0-9._\-]/g, "");
  if (!safe) return false;
  const ext = filename.split(".").pop().toLowerCase();
  if (filename.includes(".") && !ALLOWED_EXTENSIONS.has(ext)) return false;
  return true;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (createFileForm) {
    createFileForm.addEventListener("submit", async (e) => {
      ...
      if (!validateFilenameClient(filename)) {
        showToast(
          "Invalid filename. Use allowed characters and extensions.",
          "error",
        );
        return;
      }
  ...
  if (encryptionOption) {
    encryptionOption.addEventListener("change", () => {
      passcodeGroup.style.display =
        encryptionOption.value === "encrypt" ? "block" : "none";
    });
  }

  if (appLockOption) {
    appLockOption.addEventListener("change", () => {
      appLockPasscodeGroup.style.display = appLockOption.checked
        ? "block"
        : "none";
    });
  }
  ...
  if (uploadFileForm) {
    uploadFileForm.addEventListener("submit", async (e) => {
      ...
      const encrypt =
        document.getElementById("encryptionOption").value === "encrypt";
      ...
      // Client-side file validation: name/extension/size
      if (!validateFilenameClient(file.name)) {
        showToast("Invalid or unsupported filename/extension.", "error");
        return;
      }

      if (file.size > MAX_UPLOAD_SIZE) {
        showToast(
          `File is too large. Max size is ${formatBytes(MAX_UPLOAD_SIZE)}.`,
          "error",
        );
        return;
      }
      ...
      try {
        ...
        if (result.success) {
          ...
          if (result.data.encrypted)
            msg = `File encrypted and uploaded as ${result.data.filename}`;
...
async function loadFileList() {
  ...
  container.innerHTML =
    '<div class="text-center"><div class="spinner"></div><p>Loading files...</p></div>';
  ...
  try {
    ...
    if (files.length === 0) {
      container.innerHTML =
        '<div class="text-center text-muted"><p>No files found.</p></div>';
      return;
    }
...
async function getPermissions(filename) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/files/permissions/${encodeURIComponent(filename)}`,
      {
        headers: {
...
async function viewFile(filename) {
  ...
  if (user.role !== "admin" && perms.view === false) {
    showToast(
      "🚫 Permission Denied: You do not have permission to view this file",
      "error",
    );
    return;
  }
...
async function editFile(filename) {
  ...
  if (user.role !== "admin" && perms.edit === false) {
    showToast(
      "🚫 Permission Denied: You do not have permission to edit this file",
      "error",
    );
    return;
  }
